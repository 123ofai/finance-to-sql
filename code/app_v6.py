...
# AST evaluator for final formula
OPS = {
    ast.Add:  op.add,
    ast.Sub:  op.sub,
    ast.Mult: op.mul,
...
def eval_node(n, vars_):
    ...
    if isinstance(n, ast.Name):
        if n.id in vars_:
            return vars_[n.id]
        raise KeyError(f"Unknown var '{n.id}' in formula")
    if isinstance(n, ast.BinOp):
        L = eval_node(n.left, vars_)
        R = eval_node(n.right, vars_)
...
def load_resources():
    ...
    reranker_1 = CrossEncoder.from_pretrained(s1_dir)
    reranker_2 = CrossEncoder.from_pretrained(s2_dir)
...
def detect_view(nl: str) -> str:
    ...
    if re.search(rf"\bfor\b.*\b{period_unit_rx}\b", low):
        return "FTP"
    VIEW_PROTOS = FTP_KEYWORDS + PRD_KEYWORDS
    ...
    if best_score < 0.45:
        return "FTP"
    return "FTP" if best_idx < len(FTP_KEYWORDS) else "PRD"
...
all_months  = set(months + [m[:3].lower() for m in months])
quarter_caps= [f"q{i}" for i in range(1,5)] + [f"{i}st quarter" for i in range(1,5)]
...
def extract_sequence(nl: str, nature: str) -> int:
    ...
    if nature == "FQ":
        ...
        return int(m.group(1)) if m else (now.month-1)//3 + 1
    if nature == "FH":
...
# ─── FETCH METRIC ─────────────────────────────────────────────────────────────
def fetch_metric(grouping_id, period_id, nature, scenario):
    sql = f'''
SELECT value
FROM "{SCHEMA}"."{TABLE}"
...
'''
    ...
    if df.empty:
        raise ValueError(f"No data for grouping_id={grouping_id}")

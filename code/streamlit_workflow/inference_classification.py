...
# ─── DOWNLOAD & LOAD CLASSIFICATION MODEL ────────────────────────────────────
def download_prefix(prefix: str, target_dir: str):
...
CLASSIFIER_PREFIX = azure_conf["model_prefix"] + "trained_model/"
CLASSIFIER_DIR    = "/tmp/classifier"
download_prefix(CLASSIFIER_PREFIX, CLASSIFIER_DIR)
...
def load_resources():
    ...
    with SSHTunnelForwarder(
        ...
        group_df = pd.read_sql(
            f'SELECT grouping_id, grouping_label FROM "{SCHEMA}".fbi_grouping_master',
            con=engine
        )
...
def handler(event, context):
    ...
    cls_out   = classifier(query, top_k=None)[0]

...
OUTPUT_CSV = "../results/period_result_5.csv"
...
def extract_sequence(nl: str, nature: str) -> int:
    ...
    if nature == "FQ":
        ...
        for w, n in ORDINAL_MAP.items():
            if f"{w} quarter" in low: return n
    # Explicit half
    if nature == "FH":
        if "h1" in low or "first half" in low:  return 1
        if "h2" in low or "second half" in low: return 2
        for w, n in ORDINAL_MAP.items():
            if f"{w} half" in low: return n
    # FY always 1
    if nature == "FY":
        return 1
    # Relative "last N months" or "last quarter(s)"
    ...
    if re.search(r"last\s+month", low) and nature == "M":
    ...
    if m and nature == "FQ":
        ...
        seq = anchor - n
        return seq if seq >= 1 else 1
    if re.search(r"last\s+quarter", low) and nature == "FQ":
        anchor = (datetime.now().month - 1)//3 + 1
        seq = anchor - 1
        return seq if seq >= 1 else 1
    # Fallback to current

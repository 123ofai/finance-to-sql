...
def model_fn(model_dir):
    ...
    """
    # 1) Load SSH & Postgres configs from files you packaged under config/
    cfg_dir = os.path.join(model_dir, "config")
    with open(os.path.join(cfg_dir, "SSH_CONFIG.json"), "r") as f:
        ssh_conf = json.load(f)
    with open(os.path.join(cfg_dir, "PG_CONFIG.json"), "r") as f:
        pg_conf = json.load(f)

    # 2) Load your CSVs
    data_dir = os.path.join(model_dir, "data")
    gloss_df  = pd.read_csv(os.path.join(data_dir, "1b_glossary_descriptions.csv"))
    group_df  = pd.read_csv(os.path.join(data_dir, "fbi_grouping_master.csv"))

    # 3) Build text lists for embedding
    ...
    # 4) Load your models
    bi_encoder    = SentenceTransformer(os.path.join(model_dir, "models", "bi_encoder"))
    reranker_1    = CrossEncoder(os.path.join(model_dir, "models", "stage1_cross_encoder_finetuned_MiniLM_noisyhardnegative_v3_withdesc"))
    reranker_2    = CrossEncoder(os.path.join(model_dir, "models", "stage2_cross_encoder_finetuned_MiniLM_hardnegative_v2"))
    period_encoder = SentenceTransformer(os.path.join(model_dir, "models", "period_encoder"))
...
def extract_glossary(nl_query: str, resources) -> str:
...
def lookup_grouping(gloss_term: str, resources) -> (str,int):
    bi_encoder = resources['bi_encoder']
...
def predict_fn(input_data, resources):
    ...
    """
    nl_query   = input_data
    ssh_conf = resources["ssh_conf"]
    pg_conf = resources["pg_conf"]

    # 1) NL → Glossary
    gloss = extract_glossary(nl_query, resources)

    # 2) Glossary → Grouping
    label, gid = lookup_grouping(gloss, resources)

    # 3) Period, Scenario, Nature, Sequence
    period_id = construct_period_id(nl_query, resources)
